...
db = SQLAlchemy(app)
# Sno Name Phone no Messages Date Email
...
def uploader():
    if ('user' in session and session['user'] == params['admin_user']):
        if (request.method == 'POST'):
            ...
            f.save(os.path.join(app.config['UPLOAD_FOLDER'], a))
...
def contact():
    if request.method == 'POST':
        ...
        entry = Contact(name=name, Phone_num=phone, mes=message,
                        date=datetime.date.today(), email=email)
        ...
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=box_title, slug=slug,
                             content=content, tag_line=tag_line, img_file=img_file, date=date)
                db.session.add(post)
                db.session.commit()
            else:
                post = Posts.query.filter_by(sno=sno).first()
